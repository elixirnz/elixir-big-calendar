...
var _excluded$7 = ['date', 'className']
...
function getStyledEvents$2(_ref2) {
  ...
  _createClass(Event, [
    {
      ...
      value: function intersects(e2) {
        ...
        if (e2.start >= this.start && e2.start < this.end)
          return OTHER_STARTS_LATER
        return NOINT
      },
    },
  ])
...
function alignEvents(EV, _ref2) {
  ...
  var playTetrisWithEvents = function playTetrisWithEvents(EV) {
    var _iterator = _createForOfIteratorHelper(EV),
      _step
    try {
  ...
  var fillHoles = function fillHoles(eventsByLevel) {
    var intersectsWithAnyEventOnThisLevel =
      function intersectsWithAnyEventOnThisLevel(thisLevel, upperLevelEvent) {
        return (
          eventsByLevel[thisLevel].findIndex(function (e) {
  ...
  var assemblyIntersectionGraph = function assemblyIntersectionGraph(
    eventsByLevel
  ) {
    var localPeaks = []
    var numLevels = Object.keys(eventsByLevel).length
    for (var curLevel = 0; curLevel < numLevels - 1; ++curLevel) {
      var _loop = function _loop() {
        ...
        ;(_lowerEvent$children = lowerEvent.children).push.apply(
          _lowerEvent$children,
          _toConsumableArray(childEvents)
        )
        var _iterator2 = _createForOfIteratorHelper(childEvents),
          _step2
        try {
  ...
  assemblyIntersectionGraph(eventsByLevel)

  //console.log(eventsByLevel)

  var intersectWithUpperLevelEvents = function intersectWithUpperLevelEvents(
    event,
    currentLevel,
    eventsByLevel
  ) {
    ...
    for (var i = numLevels - 2; i >= 0; --i) {
      ...
      for (var j = 0; j < levelEvents.length; ++j) {
        if (levelEvents[j].localPeak) {
          ...
          var intersectingEvent = intersectWithUpperLevelEvents(
            levelEvents[j],
            i,
            eventsByLevel
          )
          ...
          var allChildrenStretchable =
            levelEvents[j].children.findIndex(function (e) {
  ...
  markStretchableEvents(eventsByLevel)
        ...
        for (_iterator5.s(); !(_step5 = _iterator5.n()).done; ) {
          ...
          if (ev.stretchable) {
            ...
            if (curLevel == 0) {
              ...
              var maxXOffset = 0
              var _iterator6 = _createForOfIteratorHelper(ev.parents),
                _step6
              try {
            ...
            var _iterator7 = _createForOfIteratorHelper(ev.children),
              _step7
            try {
            ...
            if (curLevel == 0) {
              ...
              var _maxXOffset = 0
              var _iterator8 = _createForOfIteratorHelper(ev.parents),
                _step8
              try {
                for (_iterator8.s(); !(_step8 = _iterator8.n()).done; ) {
                  var _p = _step8.value
                  _maxXOffset = Math.max(
                    _maxXOffset,
                    _p.xOffset + _p.width * (1 - eventOverlapWidth)
                  )
                }
              } catch (err) {
                _iterator8.e(err)
...
function getStyledEvents$1(_ref3) {
  ...
  var _iterator9 = _createForOfIteratorHelper(bgEvents),
    _step9
  try {
  ...
  eventsInRenderOrder = [].concat(
    _toConsumableArray(bgEvents),
    _toConsumableArray(EV)
  )
...
var DefaultAlgorithms = {
  tetris: getStyledEvents$1,
  overlap: getStyledEvents$2,
  ...
  function DayColumn() {
    ...
    _this.renderEvents = function (_ref) {
      ...
      var _this$props = _this.props,
        ...
        resizable = _this$props.resizable,
        eventOverlapWidth = _this$props.eventOverlapWidth
      ...
      var styledEvents = getStyledEvents({
        ...
        eventOverlapWidth: eventOverlapWidth,
      })
